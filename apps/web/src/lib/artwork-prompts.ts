...
export function buildBuildingPrompt(input: { building: BuildingArt; mode: "image" | "video" }): string {
  ...
  const subjectBlock = [
    ...
    `Style: matches "Rise of Kingdoms" / "Call of Dragons" art-direction — clean stylized hand-painted textures, slight cel-shading, vibrant saturated colors, soft warm-cool color contrast, readable silhouette at thumbnail size, like a polished mobile-game building-icon ripped straight from the game UI. Each building must be visually distinct in silhouette — varied proportions (tall-narrow vs wide-low vs asymmetric vs levitating), varied platform shapes (square / round / hex / irregular / none), varied massing.`,
