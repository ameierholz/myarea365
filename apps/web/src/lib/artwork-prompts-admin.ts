...
export function buildUiIconPrompt(input: { slot: UiIconSlotInput; mode: "image" | "video" }): string {
  ...
  const SUBJECT: Record<string, string> = {
    ...
    base_silhouette_own:           "pictogram silhouette of a simple friendly house — a SQUARE base with a triangular peaked roof on top, NO chimney, NO door, NO windows, NO flag, NO tower. Just the absolute simplest home-glyph (Material-Design home icon vibe). Render in a single solid teal #22D1C3 fill on transparent or pure-black background, with a clean hard outline. This represents the OWN player's base — friendly, welcoming, instantly identifiable as 'home'.",
    base_silhouette_crew_mate:     "pictogram silhouette of a friendly allied house with a tiny pennant — a SQUARE base with a triangular peaked roof, identical footprint to the own-base house, PLUS one small triangular pennant flag on a short pole emerging from the roof peak (the pennant is the only difference from the own-base glyph). NO chimney, NO windows, NO door. Render in a single solid sky-blue / cyan #5DDAF0 fill on transparent or pure-black background, with a clean hard outline. This represents a CREW-MATE's base — same family as the own-base but lighter/colder hue to read as 'allied' on the map.",
    base_silhouette_enemy:         "pictogram silhouette of a hostile fortified bunker — a SQUARE base with a slanted angular roof (NOT triangular), an aggressive arrow-like spike or single horn on top, and two narrow slit-windows on the front face. NO door, NO flag, NO friendly details. Render in a single solid hostile red #FF3344 fill on transparent or pure-black background, with a clean hard outline. This represents an ENEMY player base — threatening, fortified, instantly recognizable as 'not-friendly'. Same overall footprint as the own-base house to stay readable as a peer-base.",
    base_silhouette_crew_founder:  "pictogram silhouette of a single fortified castle keep — ONE wide rectangular building with 4-5 chunky crenellations on top (battlement teeth), and a SINGLE triangular pennant flag on a short pole emerging from the center of the top. NO side towers, NO gatehouse, NO multiple flags, NO inner detail. Render in a single solid magenta #FF2D78 fill on transparent or pure-black background. This represents the CREW-FOUNDER's seat (the crew's main castle) — bold, blocky, chess-rook-with-flag pictogram, instantly recognizable as a crew-leader landmark.",
    base_silhouette_server_don:    "pictogram silhouette of a regal throne-keep — a SQUARE/rectangular castle base with a tall stepped roof, and on top of the roof a SINGLE majestic 5-point crown silhouette taking up roughly the upper third of the shape. NO flag, NO side towers, NO windows. Render in a single solid gold #FFD700 fill on transparent or pure-black background, with a clean hard outline. This represents the DON of the city-server — the king/ruler of the entire server, unmistakably royal at a glance even at 32px.",
