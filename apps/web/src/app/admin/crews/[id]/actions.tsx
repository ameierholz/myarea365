    ...
    <div className="flex flex-col gap-2">
      ...
      <Button variant="danger" onClick={async () => {
        ...
        act("group.kick_all_non_owners", async () => {
          await sb.from("group_members").delete().eq("group_id", crewId).neq("role", "owner");
        });
      ...
      <Button variant="danger" onClick={async () => {
        ...
        act("group.quarantine", async () => {
          await sb.from("groups").update({ quarantined_at: new Date().toISOString() } as Record<string, unknown>).eq("id", crewId);
        });
      ...
      <Button variant="danger" onClick={async () => {
        ...
        act("group.reset_stats", async () => {
          await sb.from("groups").update({ total_power: 0, total_wins: 0, territory_score: 0 } as Record<string, unknown>).eq("id", crewId);
        });
