...
export async function EngagementCard() {
  ...
  const d = dau.count ?? 0, w = wau.count ?? 0, m = mau.count ?? 0;
...
export async function SignupTrendCard() {
  ...
  return (
    <Card>
      <h2 className="text-lg font-bold mb-3">📈 Neuanmeldungen (14 Tage)</h2>
...
export async function RetentionCard() {
  ...
  return (
    <Card>
      <h2 className="text-lg font-bold mb-3">🔁 Retention (Post-Signup-Walk)</h2>
...
export async function FunnelCard() {
  ...
  const steps = [
    ...
    { label: "mit 1. Walk",       value: distinctWalked, color: "#FFD700" },
    { label: "in Crew",           value: crewedRes.count ?? 0, color: "#a855f7" },
    { label: "mit Deal-Einlösung", value: dealRes.count ?? 0, color: "#FF6B4A" },
  ];
