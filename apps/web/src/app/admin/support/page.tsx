...
export default async function SupportPage() {
  ...
  const [openRes, inProgRes, resolvedRes, urgentRes] = await Promise.all([
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <Stat label="Offen" value={openRes.count ?? 0} color="#FFD700" />
        <Stat label="In Bearbeitung" value={inProgRes.count ?? 0} color="#22D1C3" />
        <Stat label="Gelöst" value={resolvedRes.count ?? 0} color="#4ade80" />
        <Stat label="Dringend" value={urgentRes.count ?? 0} color="#FF2D78" />
      </div>
