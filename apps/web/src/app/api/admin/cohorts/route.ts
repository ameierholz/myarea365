...
export async function GET(req: NextRequest) {
  ...
  // Aktivitäts-Filter (walks archived, pivot 2026-05-05) — wird im neuen Modell
  // über last_login_at oder Marsche neu definiert. Bis dahin: via last_login_at.
  if (filters.active_within_days || filters.inactive_for_days) {
    ...
    if (filters.active_within_days) q = q.gte("last_login_at", cutoff);
    else q = q.lt("last_login_at", cutoff);
  }

  // In-Crew-Filter (groups archived → crew_members)
  if (filters.in_crew != null) {
    const { data: members } = await sb.from("crew_members").select("user_id");
