...
export async function POST(req: Request) {
  ...
  if (contentType.includes("application/json")) {
    ...
    if (!["archetype", "item", "marker", "light", "pin_theme", "siegel", "potion", "rank"].includes(body.target_type)) return NextResponse.json({ error: "bad_target_type" }, { status: 400 });

    const { data: pub } = sb.storage.from("artwork").getPublicUrl(body.path);
    const publicUrl = pub.publicUrl;

    if (body.target_type !== "archetype" && body.target_type !== "item") {
