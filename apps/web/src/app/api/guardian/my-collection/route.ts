...
export async function GET() {
  ...
  const [{ data: owned }, { data: archetypes }] = await Promise.all([
    sb.from("user_guardians")
      .select("id, archetype_id, custom_name, level, xp, wins, losses, is_active, talent_points_available, acquired_at, archetype:archetype_id(id, name, emoji, rarity, guardian_type, role, base_hp, base_atk, base_def, base_spd, ability_name, image_url, video_url, faction, specialization_tags, wave_number, is_flying)")
      .eq("user_id", auth.user.id)
      .order("acquired_at"),
    sb.from("guardian_archetypes")
      .select("id, name, emoji, rarity, guardian_type, role, base_hp, base_atk, base_def, base_spd, ability_name, ability_desc, lore, image_url, video_url, faction, specialization_tags, troop_capacity_base, troop_capacity_per_level, is_flying, wave_number, released_at")
