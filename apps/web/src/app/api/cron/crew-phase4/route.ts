 ...
 *   2. tick_end_due_crew_wars  — abgelaufene Kriege beenden + Winner berechnen
 *
...
export async function GET(req: Request) {
  ...
  const [{ data: income }, { data: wars }] = await Promise.all([
    sb.rpc("tick_blackmarket_income"),
    sb.rpc("tick_end_due_crew_wars"),
  ]);
  return NextResponse.json({ ok: true, blackmarkets_ticked: income ?? 0, wars_ended: wars ?? 0 });
}
