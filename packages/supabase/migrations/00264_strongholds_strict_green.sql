-- ══════════════════════════════════════════════════════════════════════════
-- Wegelager (Strongholds): Strikt nur auf Grünflächen — kein Fallback mehr.
-- 00171 hatte einen Random-Jitter-Fallback wenn keine Grünfläche in 4km
-- gefunden wurde, was Wegelager auf Straßen/Gebäuden platzierte.
-- Pivot 2026-05-05: Suchradius erweitert + größere Mindestfläche, kein Fallback.
-- ══════════════════════════════════════════════════════════════════════════

create or replace function public.spawn_strongholds_for_plz(p_plz text, p_center_lat double precision, p_center_lng double precision)
returns int language plpgsql security definer as $$
declare
  v_active   int;
  v_to_spawn int;
  v_spawned  int := 0;
  v_lvl      int;
  v_hp       bigint;
  v_pt       geometry;
  i int;
  v_attempts int;
begin
  select count(*) into v_active from public.strongholds
   where plz = p_plz and defeated_at is null;
  v_to_spawn := greatest(0, 15 - v_active);

  for i in 1..v_to_spawn loop
    v_lvl := 1 + floor(random() * 10)::int;
    v_hp  := public.stronghold_hp_for_level(v_lvl);
    v_pt  := null;

    -- Bis zu 12 Versuche einen zufälligen Punkt in einer Grünfläche
    -- (Park/Wald/Wiese/Friedhof/Spielplatz) im Umkreis 6km zu finden.
    -- Mindestfläche 2000 m² damit's keine Mini-Grünstreifen sind.
    v_attempts := 0;
    while v_pt is null and v_attempts < 12 loop
      v_attempts := v_attempts + 1;
      with cand as (
        select geom from public.green_areas
         where st_dwithin(
           geom::geography,
           st_setsrid(st_makepoint(p_center_lng, p_center_lat), 4326)::geography,
           6000
         )
         and area_sqm >= 2000
         order by random() limit 1
      )
      select st_pointonsurface(geom) into v_pt from cand;
    end loop;

    -- Kein Fallback: lieber kein Wegelager als eines auf einer Hauptstraße.
    if v_pt is not null then
      insert into public.strongholds (plz, lat, lng, level, total_hp, current_hp)
      values (p_plz, st_y(v_pt), st_x(v_pt), v_lvl, v_hp, v_hp);
      v_spawned := v_spawned + 1;
    end if;
  end loop;
  return v_spawned;
end $$;
