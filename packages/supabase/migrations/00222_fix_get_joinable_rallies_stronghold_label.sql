-- ─── 00222: Fix get_joinable_rallies — strongholds.name existiert nicht ──
-- Bug: 00201 referenziert s.name im stronghold-Subquery → 42703 column does not exist.
-- Fix: Label aus PLZ + Level konstruieren.
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'stronghold_id', s.id,
    'leader_name', coalesce(u.display_name, u.username),
    'target_label', 'Wegelager ' || coalesce(s.plz, '') || ' (Lvl ' || s.level || ')',
