-- 00382_resolve_attack_with_specialty_and_walls.sql
-- resolve_player_base_attack erweitert:
--   - Wächter-Specialty-Mult (+15 % bei passendem Wetter) auf atk_power UND def_power
--   - Eis-Wände: _weather_base_def_bonus_pct multiplikativ auf def_power
--   - Schlachtbericht erweitert um Specialty-Hinweis und Wetter-Def-Bonus
...
declare
  ...
  v_atk_ug_id uuid;
  v_def_ug_id uuid;
  v_atk_spec_mult numeric := 1.0;
  v_def_spec_mult numeric := 1.0;
  v_def_bonus_pct int := 0;
  v_specialty_line text := '';
begin
  ...
  if a.attacker_guardian_archetype_id is not null then
    select id into v_atk_ug_id
      from public.user_guardians
     where user_id = a.attacker_user_id and archetype_id = a.attacker_guardian_archetype_id and is_active = true
     limit 1;
    v_atk_spec_mult := public._guardian_specialty_mult(a.attacker_user_id, v_atk_ug_id, v_city_slug);
  end if;
  select id into v_def_ug_id
    from public.user_guardians where user_id = a.defender_user_id and is_active = true limit 1;
  v_def_spec_mult := public._guardian_specialty_mult(a.defender_user_id, v_def_ug_id, v_city_slug);

  v_def_bonus_pct := public._weather_base_def_bonus_pct(v_city_slug);
  ...
  for k, v_cnt in select * from jsonb_each_text(a.troops_committed) loop
    ...
    v_atk_power := v_atk_power + round(
      ...
      coalesce(public._tod_atk_mult(v_t.troop_class), 1.0) *
      coalesce(v_atk_spec_mult, 1.0)
    )::int;
  end loop;

  select coalesce(sum(round(ut.count * t.base_def *
      coalesce(public._weather_def_mult(v_city_slug, t.troop_class), 1.0) *
      coalesce(public._tod_def_mult(t.troop_class), 1.0) *
      coalesce(v_def_spec_mult, 1.0))::int), 0)::int
    into v_def_power
  ...
  if v_def_bonus_pct > 0 then
    v_def_power := round(v_def_power * (1.0 + v_def_bonus_pct::numeric / 100.0))::int;
  end if;
  ...
  select coalesce(display_name, username, 'Unbekannt') into v_atk_name from public.users where id = a.attacker_user_id;
  select coalesce(display_name, username, 'Unbekannt') into v_def_name from public.users where id = a.defender_user_id;
  ...
  if v_weather is not null and v_weather.condition not in ('clear','cloud') then
    v_weather_line := chr(10) || chr(10) || '🌦️ Wetter in ' || v_city_slug || ': ' ||
      case v_weather.condition
        when 'rain' then '🌧️ Regen'
        when 'storm' then '⛈️ Sturm'
        when 'snow' then '❄️ Schnee'
        when 'heat' then '🔥 Hitze'
        when 'fog' then '🌫️ Nebel'
        when 'night' then '🌙 Nacht'
        else v_weather.condition
      end || ' · ' || v_weather.temperature_c || '°C';
    if v_def_bonus_pct > 0 then
      v_weather_line := v_weather_line || chr(10) ||
        '🧊 Wetter-Verteidigungsbonus: +' || v_def_bonus_pct || ' %';
    end if;
  end if;

  if v_atk_spec_mult > 1.0 or v_def_spec_mult > 1.0 then
    v_specialty_line := chr(10) ||
      case when v_atk_spec_mult > 1.0 then '🌟 Dein Wächter-Spezialtalent passt zum Wetter: +15 %' else '' end ||
      case when v_def_spec_mult > 1.0 then chr(10) || '🌟 Gegner-Wächter-Spezialtalent passt: +15 %' else '' end;
  end if;

  v_tod_line := chr(10) || '🕒 Tageszeit: ' ||
    case v_tod
      when 'morning' then '🌅 Morgen'
      when 'day' then '☀️ Tag'
      when 'evening' then '🌇 Abend'
      else '🌙 Nacht'
    end;
  ...
  v_payload_atk := jsonb_build_object(
    ...
    'weather', to_jsonb(v_weather), 'tod', v_tod,
    'specialty_atk_mult', v_atk_spec_mult, 'specialty_def_mult', v_def_spec_mult,
    'def_weather_bonus_pct', v_def_bonus_pct,
  ...
  v_atk_body := 'Ergebnis: ' || v_outcome_label_atk ||
    ...
    v_atk_g_line || v_def_g_line || v_weather_line || v_specialty_line || v_tod_line ||
    chr(10) || chr(10) || 'Beute:' ||
  ...
  v_def_body := 'Ergebnis: ' || v_outcome_label_def ||
    ...
    replace(v_def_g_line, 'Gegner-Wächter', 'Dein Wächter') || v_weather_line || v_specialty_line || v_tod_line ||
