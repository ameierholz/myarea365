...
values
  -- ═══ COMMON (5) ═════════════════════════════════════════════════════════
  ('stein_waechter',    'Stein-Wächter',      '🗿', 'common', 'tank',    'infantry', 'ritter',        'construct', 'neutral',
    110, 14, 18, 12, 'wall',     'Bollwerk',     '+20% DEF in der ersten Runde',          'Aus dem Fels der alten Stadtmauer geschnitten — er kennt jeden Stein.'),
  ('kanal_spaeherin',   'Kanal-Späherin',     '🐀', 'common', 'melee',   'cavalry',  'schurke',       'beast',     'female',
     85, 18, 10, 26, 'ambush',   'Ratten-Biss',  '+50% ATK bei erstem Treffer',           'Kennt jeden Tunnel — und jede Beute.'),
  ('markt_heilerin',    'Markt-Heilerin',     '🌿', 'common', 'support', 'mage',     'kleriker',      'human',     'female',
     95, 14, 14, 18, 'focus',    'Kräuterkraft', 'Ignoriert 50% DEF bei kritischen Treffern','Ihre Tinkturen halten den Kiez am Leben.'),
  ('dorf_bogner',       'Dorf-Bogner',        '🏹', 'common', 'ranged',  'marksman', 'bogenschuetze', 'human',     'male',
     90, 17, 12, 20, 'focus',    'Ruhiger Atem', 'Ignoriert 50% DEF bei kritischen Treffern','Hat mehr Pfeile gezählt als Sterne gesehen.'),
  ('gluehpilz_schwarm', 'Glühpilz-Schwarm',   '🍄', 'common', 'ranged',  'mage',     'hexer',         'spirit',    'neutral',
     80, 15, 12, 22, 'poison',   'Sporenwolke',  '5% HP des Gegners pro Runde (max 3)',   'Eine schwebende Sporenwolke — manchmal heilt sie, manchmal zersetzt sie.'),

  -- ═══ ELITE (5) ══════════════════════════════════════════════════════════
  ('eisen_klingerin',   'Eisen-Klingerin',    '⚔️', 'elite', 'melee',   'cavalry',  'samurai',       'human',     'female',
    120, 24, 16, 26, 'ambush',   'Doppelhieb',   '+50% ATK bei erstem Treffer',           'Zwei Klingen, ein Atemzug — und du blutest schon.'),
  ('frost_druidin',     'Frost-Druidin',      '❄️', 'elite', 'support', 'mage',     'schamane',      'elf',       'female',
    110, 20, 18, 22, 'rebirth',  'Frost-Atem',   'Einmal pro Kampf: voll geheilt bei 0 HP', 'Sie atmet die Kälte des Polarsterns.'),
  ('ork_wachhund',      'Ork-Wachhund',       '🛡️', 'elite', 'tank',    'infantry', 'krieger',       'orc',       'male',
    140, 22, 24, 14, 'fortress', 'Stahlkiefer',  '+30% DEF wenn Arena in eigener Stadt',  'Drei Tonnen Muskel und ein einziger Befehl: HALT.'),
  ('sturm_bogner',      'Sturm-Bogner',       '🪶', 'elite', 'ranged',  'marksman', 'bogenschuetze', 'bird',      'male',
    105, 25, 15, 28, 'echolot',  'Sturzschuss',  'Trifft immer zuerst in Runde 1',        'Federvolk vom Hochgebirge — er schießt mit dem Wind.'),
  ('gossen_geist',      'Gossen-Geist',       '👻', 'elite', 'melee',   'cavalry',  'ninja',         'spirit',    'neutral',
    100, 24, 13, 30, 'stealth',  'Schattenfaust','25% Chance einen Angriff zu verdoppeln','Ein Echo aus den unteren Etagen — er war nie ganz fort.'),

  -- ═══ EPIC (5) ═══════════════════════════════════════════════════════════
  ('magma_titan',       'Magma-Titan',        '🌋', 'epic',  'tank',    'infantry', 'berserker',     'construct', 'neutral',
    200, 28, 32, 14, 'fortress', 'Glühender Kern','+30% DEF wenn Arena in eigener Stadt', 'Die Adern fließen mit flüssigem Gestein.'),
  ('plasma_magierin',   'Plasma-Magierin',    '⚡', 'epic',  'ranged',  'mage',     'magier',        'human',     'female',
  ...
  ('voodoo_schamane',   'Voodoo-Schamane',    '🐸', 'epic',  'support', 'mage',     'schamane',      'beast',     'male',
    150, 26, 22, 22, 'poison',   'Hex-Brühe',    '5% HP des Gegners pro Runde (max 3)',   'Sein Lachen klingt wie Quaken — sein Fluch wirkt wie Stein.'),
  ('schatten_assassinin','Schatten-Assassinin','🗡️', 'epic',  'melee',   'cavalry',  'ninja',         'elf',       'female',
    135, 32, 18, 30, 'stealth',  'Klingen-Tanz', '25% Chance einen Angriff zu verdoppeln','Drei Schritte vorwärts, ein Schritt zurück — du bist tot.'),
  ('sand_berserker',    'Sand-Berserker',     '🗡️', 'epic',  'melee',   'cavalry',  'schurke',       'desert',    'male',
    160, 30, 24, 22, 'rage',     'Wüstenwut',    '+5% ATK pro erlittenem Treffer (max +40%)','Je mehr du ihn schlägst, desto mehr Wüste atmet er ein.'),

  -- ═══ LEGENDARY (5) ══════════════════════════════════════════════════════
  ('drachen_ritterin',  'Drachen-Ritterin',   '🐉', 'legendary','ranged','marksman','bogenschuetze', 'dragonkin', 'female',
    180, 38, 26, 30, 'firststrike','Drachen-Speer','Erste Runde: +100% ATK, immer kritisch','Halb Drache, halb Mensch — ganz Krieg.'),
  ('lichtwaechter',     'Ewiger Lichtwächter','✨', 'legendary','support','mage',    'priester',      'celestial', 'neutral',
  ...
  ('knochen_koenig',    'Knochen-König',      '💀', 'legendary','tank',   'infantry','paladin',       'undead',    'male',
    240, 32, 36, 16, 'fortress', 'Untote Krone', '+30% DEF wenn Arena in eigener Stadt',  'Tot war er gestern. Heute ist er König.'),
  ('vulkan_klinge',     'Vulkan-Klinge',      '🔥', 'legendary','melee',  'cavalry', 'samurai',       'demon',     'male',
    200, 42, 28, 24, 'flame',    'Lava-Hieb',    'Gegner verliert 10% HP pro Runde (ignoriert DEF)','Ein Hieb genügt — der Rest brennt von selbst.'),
  ('sternensaengerin',  'Sternensängerin',    '🌌', 'legendary','ranged', 'marksman','runenmeister',  'cosmic',    'female',
