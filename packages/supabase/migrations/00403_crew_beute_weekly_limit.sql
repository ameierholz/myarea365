-- ────────────────────────────────────────────────────────────────────────
-- 00403: 4×/Woche-Limit für Spende + Donor-Name in list_crew_gifts
-- ────────────────────────────────────────────────────────────────────────
-- 4 Spenden pro rollendem 7-Tage-Fenster pro User.
-- list_crew_gifts gibt zusätzlich aus, wie viele Spenden der Caller diese
-- Woche noch hat, plus Donor-Name pro pending-Gift damit die Card
-- den Namen vom Spender prominent rendern kann.
-- ────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Limit: 4 Spenden pro rollendem 7-Tage-Fenster
  SELECT COUNT(*) INTO v_week_count FROM public.crew_gifts
    WHERE source_user_id = v_uid AND source = 'shop' AND created_at > now() - interval '7 days';
  IF v_week_count >= 4 THEN RAISE EXCEPTION 'weekly_limit_reached'; END IF;

  IF p_cost >= 1000 THEN v_tier := 'mega'; v_key_pts := 1500; v_crys_pts := 750;
  ELSE v_tier := 'elite'; v_key_pts := 500; v_crys_pts := 250; END IF;
  ...
  END LOOP;
  RETURN jsonb_build_object('ok', true, 'gift_id', v_gift_id, 'gems_spent', p_cost, 'donor', v_donor, 'tier', v_tier, 'weekly_used', v_week_count + 1, 'weekly_remaining', 3 - v_week_count);
END $function$;
...
-- list_crew_gifts: donor_name pro pending-Gift + weekly_used/remaining
...
BEGIN
  ...
  -- Wochen-Counter eigener Spenden
  SELECT COUNT(*) INTO v_week_count FROM public.crew_gifts
    WHERE source_user_id = v_uid AND source = 'shop' AND created_at > now() - interval '7 days';
  ...
  RETURN jsonb_build_object('pending', v_pending, 'claimed_recent', v_claimed,
    ...
    'donation_limits', jsonb_build_object('weekly_used', v_week_count, 'weekly_limit', 4, 'weekly_remaining', GREATEST(0, 4 - v_week_count)));
END $function$;
