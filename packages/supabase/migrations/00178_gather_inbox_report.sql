-- ══════════════════════════════════════════════════════════════════════════
-- Sammel-Marsch: Inbox-Report bei Rückkehr
-- Erweitert tick_gather_marches() um eine Inbox-Nachricht (category=report,
-- kind='gather_complete') sobald die Truppen wieder zu Hause sind.
-- ══════════════════════════════════════════════════════════════════════════
...
begin
  ...
  -- returning → completed: Inbox-Report schreiben
  for v_m in select * from public.gather_marches where status = 'returning' and v_now >= returns_at loop
    ...
    values (
      ...
      'Sammel-Trupp zurück: ' || coalesce(v_node.name, v_kind_label),
